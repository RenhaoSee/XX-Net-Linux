...
def send_response(wfile, status=404, headers={}, body=''):
    ...
    try:
        ...
        logger.warn("send response fail")
...
def pack_request(method, url, headers, body, timeout):
    ...
    if isinstance(body, basestring) and body:
        ...
        if len(body) > 10 * 1024 * 1024:
            logger.warn("body len:%d %s %s", len(body), method, url)
...
def request_gae_server(headers, body, url, timeout):
    ...
    try:
        ...
        if response.status == 503:
            logger.warning('APPID %r out of Quota, remove it. %s',
                         appid, response.ssl_sock.ip)
        ...
        if ("gws" not in server_type and "Google Frontend" not in server_type and "GFE" not in server_type) or \
            ...
            logger.warn("IP:%s not support GAE, headers:%s status:%d", response.ssl_sock.ip, response.headers,
                      response.status)
...
def request_gae_proxy(method, url, headers, body, timeout=None):
    ...
    if "br" in accept_encoding:
        ...
        # logger.debug("accept_br_encoding for %s", url)
    else:
    ...
    for timeout in timeouts:
        ...
        try:
            ...
            # logger.debug("accept:%s content-encoding:%s url:%s", accept_encoding,
            ...
            if not accept_br_encoding:
                ...
                if response.headers.get("Content-Encoding", "") == "br":
                    ...
                    # logger.debug("remove br from Content-Encoding, %s", url)
                    if host not in front.config.br_sites:
                        front.config.BR_SITES.append(host)
                        front.config.save()
                        front.config.load()
                        logger.warn("Add %s to br_sites", host)

            if response.app_msg:
                logger.warn("server app return fail, status:%d",
                          response.app_status)
                # if len(response.app_msg) < 2048:
                # logger.warn('app_msg:%s', cgi.escape(response.app_msg))
            ...
            #logger.warn("gae_exception:%r %s", e, url)
            #if e.message == '605:status:500':                raise e
        except Exception as e:
            pass
            #err_msg = 'gae_handler.handler %r %s , retry...' % (e, url)
            #error_msg.append(err_msg)
            #logger.exception('gae_handler.handler %r %s , retry...', e, url)
...
def handler(method, host, url, headers, body, wfile, fallback=None):
    ...
    if method == "GET":
        if req_range_begin and not req_range_end:
            ...
            logger.debug("change Range %s => %s %s",
            ...
            if decoded_data1:
                try:
                    ...
                    if "deflate" in headers.get("Accept-Encoding", ""):
                        ...
                        logger.info("GAE send ungziped deflate data to browser t:%d s:%d %s %s %s", (time.time() - request_time) * 1000, content_length, method,
                        ...
                        while True:
                            ...
                            decoded_data0 = gzip_decompressor.decompress(data)
                            decoded_data1 = deflate_decompressor.decompress(decoded_data0)
                        logger.info("GAE send ungziped data to browser t:%d s:%d %s %s %s", (time.time() - request_time) * 1000, content_length, method,
                                  url, response.task.get_trace())

                    return
                except Exception as e:
                    logger.info("gae_handler.handler try decode and send response fail. e:%r %s", e, url)
                    return
    ...
    while True:
        ...
        try:
            ...
            if e_b[0] in (errno.ECONNABORTED, errno.EPIPE,
                ...
                logger.info('gae_handler send to browser return %r %r, len:%d, sended:%d', e_b, url, body_length, body_sended)
            else:
                logger.info('gae_handler send to browser return %r %r', e_b, url)
            return

    # 完整一次https请求
    logger.info("GAE t:%d s:%d %s %s %s", (time.time() - request_time) * 1000, content_length, method, url,
            ...
            with self.lock:
                ...
                if self.req_begin > self.wait_begin + front.config.AUTORANGE_MAXSIZE:
                    if self.get_all_buffer_size() > front.config.AUTORANGE_MAXBUFFERSIZE * (0.8 + len(self.all_data_size) * 0.2):
                        if not self.blocked:
                            logger.debug("fetch_worker blocked, buffer:%d %s",
    ...
    def fetch(self, begin, end, first_response):
        ...
        while self.keep_running:
            ...
            if response.headers.get('Location', None):
                ...
                logger.warn('RangeFetch Redirect(%r) status:%s',
            ...
            if not content_range:
                logger.warning('RangeFetch "%s %s" return headers=%r, retry %s-%s',
            ...
            while True:
                if data_readed >= content_length:
                    ...
                    logger.debug('RangeFetch [%s] %d%% length:%s range:%s %s %s',
                ...
                if data_len > expect_len:
                    logger.warn("RangeFetch expect:%d, get:%d",
                              expect_len, data_len)
